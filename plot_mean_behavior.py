...
def assign_episode_number(data_df, episodes):
...
def get_episode_start_window(data_df, episodes, window_period=(-5, 5)):
    ...
    for ep in episodes:
        ...
        # Always reference the start of the window to the beginning of the episode
...
def plot_individual_behaviors(data_dict, f_trace='zscore', channel_key=None, plot_singles=False):
    ...
    for k in data_dict.keys():
        ...
        if len(f_traces_of_key) > 0:
            ...
            trace_array = f_util.remove_baseline(t, trace_array, norm_start=-3, norm_end=0)
...
def plot_multiple_behaviors(data_dict, keys_to_plot, f_trace='zscore', channel_key=None, plot_singles=True):
...
if __name__ == "__main__":
    ...
    behaviors_to_analyze = ['Eating Zone', 'Eating']
    period = (-3,  10)    # In seconds
