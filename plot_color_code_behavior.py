...
def main(data, channel_key=None):
    """
...
if __name__ == "__main__":
    ...
    if 'behavior' in data.columns:
        ...
        try:
            ...
            behavior_bouts, zone_bouts = f_util.find_zone_and_behavior_episodes(data, behavior_labels)
            data = f_util.add_episode_data(data, behavior_bouts, zone_bouts)
            fig = main(data)
    ...
    plt.savefig(join(paths.figure_directory, "_".join(('animal{}_day{}'.format(mouse, day), 'zdff', 'behavior_seg')) + ".png"))
    plt.show()
